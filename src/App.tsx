...
import { LeadHunterTab } from "./tabs/LeadHunterTab";
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {user.role === "ADMIN" && view === "overview" ? <DashboardTab loading={loading} stats={stats} tenants={tenants} /> : null}

        {user.role === "OWNER" ? (
          loading ? (
            <section className="kpi-grid">
              {Array.from({ length: 4 }, (_, idx) => (
                <section className="panel skeleton-card" key={`owner-kpi-skeleton-${idx}`}>
        ...
        {user.role === "ADMIN" && view === "tenants" ? (
          <TenantsTab
            ...
            onDetails={openTenantDetails}
            onEdit={openEditTenantModal}
            onOwner={openManageOwnerModal}
            onRenew={handleManualRenew}
            onSchedulePrice={handleSchedulePrice}
            pricingDrafts={pricingDrafts}
            setDueDayFilter={setDueDayFilter}
            setPricingDrafts={setPricingDrafts}
            setTenantSearch={setTenantSearch}
            tenantSearch={tenantSearch}
            tenants={filteredTenants}
            toneFromLicenseStatus={toneFromLicenseStatus}
          />
        ) : null}

        {user.role === "ADMIN" && view === "lead_hunter" ? (
          <LeadHunterTab
            handleCopyLeadMessage={handleCopyLeadMessage}
            handleCreateLeadCampaign={handleCreateLeadCampaign}
            ...
            leadConsentOptions={leadConsentOptions}
            leadForm={leadForm}
            leadOpportunities={filteredLeadOpportunities}
            leadSourceOptions={leadSourceOptions}
            leadStats={leadStats}
            leadStatusFilter={leadStatusFilter}
            leadStatusOptions={leadStatusOptions}
            loading={loading}
            setLeadCampaignForm={setLeadCampaignForm}
            setLeadForm={setLeadForm}
            setLeadStatusFilter={setLeadStatusFilter}
            toneFromLeadStatus={toneFromLeadStatus}
            toneFromLeadTemperature={toneFromLeadTemperature}
          />
        ) : null}
